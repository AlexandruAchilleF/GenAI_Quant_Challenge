...
export default function ChatPane() {
  ...
  return (
    <div className="flex flex-col h-full bg-surface-900/50">
      {/* Header */}
      <div className="h-12 flex items-center justify-between px-4 border-b border-surface-800 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-surface-200">AI Chat</span>
        </div>
        {messages.length > 0 && (
          <button
      ...
      <div className="flex-1 overflow-y-auto">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full px-6 text-center">
            ...
            <h3 className="text-sm font-semibold text-surface-400 mb-1">Ask me anything</h3>
            <p className="text-xs text-surface-600 max-w-[240px]">
            ...
            <div className="mt-6 space-y-1.5 w-full">
              {[
                ...
                <button
                  ...
                  className="w-full text-left px-3 py-2 rounded-lg text-xs text-surface-400 hover:text-surface-200 hover:bg-surface-800/60 transition-all border border-surface-800/50 hover:border-surface-700"
                >
                  {qp}
                </button>
              ))}
      ...
      <div className="p-3 border-t border-surface-800 flex-shrink-0">
        <div className="flex gap-2">
          <input
            ...
            disabled={isThinking}
            className="flex-1 bg-surface-800/50 border border-surface-700 rounded-xl px-3 py-2.5 text-sm text-surface-200 placeholder-surface-600 focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500/50 transition-all disabled:opacity-50"
          />
          <button
            onClick={() => handleSend()}
            disabled={!input.trim() || isThinking}
            className="w-10 h-10 rounded-xl bg-gradient-to-r from-primary-600 to-primary-500 text-white flex items-center justify-center hover:from-primary-500 hover:to-primary-400 transition-all disabled:opacity-40 disabled:cursor-not-allowed active:scale-95"
          >
            <Send className="w-4 h-4" />
          </button>
        </div>
        <p className="text-[10px] text-surface-600 mt-1.5 px-1">
          Press <kbd className="px-1 py-0.5 bg-surface-800 rounded text-surface-500 font-mono text-[9px]">Enter</kbd> to send · Try "rename X to Y" or "make X blue"
        </p>
      </div>
    </div>
  )
