...
export default function ThinkingBlock({ steps = [], isStreaming = false }) {
  ...
  return (
    <div className="my-2 rounded-xl border border-surface-700/50 bg-surface-800/30 overflow-hidden">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-surface-400 hover:text-surface-300 transition-colors"
      >
      ...
      {isOpen && (
        <div className="px-3 pb-3 space-y-1.5">
          {steps.map((step, i) => (
            ...
            >
              <span className="w-4 h-4 rounded-full bg-primary-500/20 flex items-center justify-center flex-shrink-0 mt-0.5">
